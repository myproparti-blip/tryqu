...
export default function CheckoutPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <div className="sticky top-0 z-50 liquid-glass border-b border-white/10 backdrop-blur-md">
        <div className="px-4 py-3 sm:px-6 sm:py-4">
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            <Button
              ...
              size="sm"
              onClick={handleBack}
              className="p-2 hover:bg-white/10 rounded-full -ml-2 sm:p-2.5 text-white"
            >
              {currentStep === 1 ? (
            ...
            <div className="flex items-center justify-between flex-1 mx-3 sm:mx-4">
              ...
              <div className="text-base font-bold text-lime-300 sm:text-lg">{formatPrice(calculateTotal())}</div>
            </div>
          ...
          <div className="h-0.5 bg-neutral-800 rounded-full overflow-hidden sm:h-1">
            <div
              className="h-full bg-lime-300 rounded-full transition-all duration-500 ease-out shadow-lg shadow-lime-300/20"
            ...
            <div className="space-y-8 sm:space-y-10">
              ...
              <Card className="border-neutral-800 liquid-glass rounded-xl sm:rounded-2xl">
                <CardContent className="p-5 space-y-4 sm:p-8 sm:space-y-6">
                  ...
                  <div className="flex justify-between items-center py-4 bg-lime-300/10 rounded-xl px-4 mt-4 sm:py-6 sm:rounded-2xl sm:px-6 sm:mt-6 border border-lime-300/20">
                     <h4 className="text-lg font-bold text-white sm:text-xl">Total</h4>
                     <span className="text-2xl font-bold text-lime-300 sm:text-3xl">
              ...
              <div className="space-y-4 sm:space-y-6">
                {stepContent.options?.map((option) => (
                  <Button
                    ...
                    variant="outline"
                    className="w-full h-auto p-4 text-left liquid-glass border border-white/20 hover:border-lime-300/50 hover:bg-lime-300/5 transition-all duration-200 group rounded-2xl sm:p-6 bg-transparent"
                  >
                    <div className="flex items-center gap-4 w-full sm:gap-5">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <div className="font-semibold text-white text-base group-hover:text-lime-300 transition-colors sm:text-lg">
                             {option.title}
                           </div>
                           {option.price && (
                             <div className="text-lime-300 font-bold text-sm sm:text-base">{option.price}</div>
                           )}
                        </div>
                        {option.subtitle && (
                      ...
                      <ArrowRight className="h-5 w-5 text-neutral-600 group-hover:text-lime-300 transition-colors flex-shrink-0 sm:h-6 sm:w-6 sm:ml-3" />
                    </div>
                  </Button>
                ))}
              </div>
      ...
      {/* Premium Bottom CTA */}
      {stepContent.isSummary && (
        <div className="fixed bottom-0 left-0 right-0 liquid-glass border-t border-white/10 backdrop-blur-md p-4 sm:p-8">
          <div className="max-w-sm mx-auto sm:max-w-md space-y-3">
            {/* Preview button for testing */}
            <Button
              onClick={previewWhatsAppMessage}
              variant="outline"
              className="w-full h-10 text-sm border-white/20 text-neutral-300 hover:bg-white/10 bg-transparent rounded-lg"
            >
              Preview Message
            </Button>
            ...
            <Button
              ...
              className="w-full h-12 text-base font-semibold bg-lime-300 text-black hover:bg-lime-400 rounded-lg shadow-lg shadow-lime-300/30 transition-all sm:h-16 sm:text-xl sm:rounded-xl"
            >
